...
interface DownloadCreditNotePdfInput {
  creditNoteId: string;
...
export function useDownloadCreditNotePdf() {
  const mutation = useMutation({
    mutationFn: async ({
      creditNoteId,
      creditNoteNumber,
    }: DownloadCreditNotePdfInput) => {
      const res = await api["credit-notes"]({ id: creditNoteId }).pdf.get();
    ...
    onSuccess: ({ fileName, pdfBase64, creditNoteNumber }) => {
      ...
      anchor.download =
        sanitizeFileName(
          fileName?.trim() || `nota-credito-${creditNoteNumber}.pdf`,
        ) || `nota-credito-${creditNoteNumber}.pdf`;
