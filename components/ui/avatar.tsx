...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        "group/avatar after:border-border relative flex size-8 shrink-0 select-none after:absolute after:inset-0 after:border after:mix-blend-lighten data-[size=lg]:size-10 data-[size=sm]:size-6",
        className,
      )}
      {...props}
    />
...
function AvatarFallback({
  ...
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn(
        "bg-muted text-muted-foreground flex size-full items-center justify-center font-sans text-sm group-data-[size=sm]/avatar:text-xs",
        className,
      )}
      {...props}
    />
