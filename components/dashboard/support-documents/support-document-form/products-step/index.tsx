...
import type {
  SupportDocumentFormValues,
  SupportDocumentItemValues,
...
interface SelectedLine {
  index: number;
  item: SupportDocumentItemValues;
}
...
const ProductsStep: FC = () => {
  const { control, setValue, trigger } =
    useFormContext<SupportDocumentFormValues>();
  ...
  const onAddProduct = (item: SupportDocumentItemValues) => {
  ...
  const onEditProduct = (index: number, item: SupportDocumentItemValues) => {
    const duplicated = items.some(
    ...
    <div className="flex flex-1 flex-col gap-6">
      <PageHeader size="sm">
        <PageHeaderContent>
          ...
          <PageHeaderDescription>
            Agrega los productos o servicios que incluirá el documento soporte
          </PageHeaderDescription>
        </PageHeaderContent>
        <PageHeaderActions>
        ...
        <Empty fillSpace className="bg-background/30">
          <EmptyHeader>
            ...
            <EmptyDescription className="max-w-[330px]">
              Agrega los productos o servicios a documentar
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button type="button" variant="secondary" onClick={openAdd}>
